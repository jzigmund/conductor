  ...
  def validate_references_in_deployable_xml(deployable_xml)
    deployable_xml.dependency_graph.not_existing_references.each do |reference|
      ...
      if reference[:no_return_param]
        errors.add(:xml,
                   _("Assembly %s, service %s, parameter %s references param %s which is not returned by Assembly %s") % [locale_params[:from_assembly], locale_params[:from_service], locale_params[:from_param], locale_params[:to_param], locale_params[:to_assembly]] )
      elsif reference[:reference][:service]
        errors.add(:xml,
                   _("Assembly %s, service %s, parameter %s references not existing Assembly %s, service %s") % [locale_params[:from_assembly], locale_params[:from_service], locale_params[:from_param], locale_params[:to_assembly], locale_params[:to_service]] )
      else
        errors.add(:xml,
                   _("Assembly %s, service %s, parameter %s references not existing Assembly %s") % [locale_params[:from_assembly], locale_params[:from_service], locale_params[:from_param], locale_params[:to_assembly]])
      end
    end
  end
    ...
    deployable_xml.assemblies.each do |assembly|
      ...
      if !error.nil?
        ...
        if image.environment != pool_family.name
          deployable_errors << _("The Deployable '%s' contains an image (UUID %s) in the wrong Environment ('%s' should be '%s') and cannot be used.") % [name, assembly.image_id, image.environment, pool_family.name]
        end
        images << image
      ...
      if assembly.hwp
        ...
        deployable_errors << "#{assembly_hash[:name]}: " + _("invalid")
      end
